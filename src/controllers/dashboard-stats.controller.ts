...
import { calculatePercantage, getCategories, getChartData, MyDocumnet } from "../utils/features.js";
...
export const getDashboardStats = TryCatch(async (req, res, next) => {
  ...
  else 
  {
    ...
    const thisMonthRevanue = thisMonthOrders.reduce(
      (total, order) => total + (order.total || 0),
      0
    );

    const lastMonthRevanue = lastMonthOrders.reduce(
      (total, order) => total + (order.total || 0),
      0
    );
    ...
    const percent = {
      ...
      userPercent: calculatePercantage(
        thisMonthUsers.length,
        lastMonthUsers.length
      ),
      productPercent: calculatePercantage(
        thisMonthProducts.length,
        lastMonthProducts.length
      ),
      orderPercent: calculatePercantage(
        thisMonthOrders.length,
        lastMonthOrders.length
      ),
    };
    const revanue=allOrders.reduce(
    (total, order) => total + (order.total || 0),
    0
  );
...
export const getDashboardBar = TryCatch(async (req, res, next) => {
   ...
   const productCounts= getChartData({length:6,docArr:products as unknown as MyDocumnet[],today});
   const ordersCounts=getChartData({length:12,docArr:orders as unknown as MyDocumnet[],today})
   const usersCounts=getChartData({length:6,docArr:users as unknown as MyDocumnet[],today})
...
export const getDashboardLine = TryCatch(async (req, res, next) => 
{
  ...
  else
  {
   ...
   const [orders,users,products]= await Promise.all([
    ...
    const userCount=getChartData({length:12,docArr:users as unknown as MyDocumnet[],today});
    
    const productCount=getChartData({length:12,docArr:products as unknown as MyDocumnet[],today});
    
    const discount=getChartData({length:12,docArr:orders as unknown as MyDocumnet[],today,property:"discount",});

    const revanue=getChartData({length:12,docArr:orders as unknown as MyDocumnet[],today,property:"total",});
